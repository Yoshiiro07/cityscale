<?php
	...
	##UPLOAD
	if ($lrequest == "Upload"){
		$sql = "REPLACE INTO SaveData (AccountID,Name,Sex,Level,Map,Exp,Job,Hp,Mp,Money,HpMax,MpMax,regenTime,regenTimeMax,PosX,PosY,Walk,Frame,Target,AtkTimer,Casting,Atk,Def,Evasion,Side,Set,Hair,Color,Hat,Weapon,Crystal1,Crystal2,Crystal3,Crystal4,Crystal5,StatusPoint,Str,Agi,Vit,Dex,Wis,Luk,Res,Stamina,StaminaMax,StaminaTimer,Itens,Quests,hotkey1,hotkey2,buffA,BuffTimeA,buffB,BuffTimeB,buffC,BuffTimeC,party,playerInBattle,playerInAttack,playerInCast,pet,pethungry,petcare,petTraining,petBath,petLevel) VALUES ('$lAccountID','$lName','$lSex','$lLevel','$lMap','$lExp','$lJob','$lHp','$lMp','$lMoney','$lHpMax','$lMpMax','$lregenTime','$lregenTimeMax','$lPosX','$lPosY','$lWalk','$lFrame','$lTarget','$lAtkTimer','$lAtkTimerMax','$lCasting','$lAtk','$lDef','$lEvasion','$lSide','$lSet','$lHair','$lColor','$lHat','$lWeapon','$lCrystal1','$lCrystal2','$lCrystal3','$lCrystal4','$lCrystal5','$lStatusPoint','$lStr','$lAgi','$lVit','$lDex','$lWis','$lLuk','$lRes','$lStamina','$lStaminaMax','$lStaminaTimer','$lItens','$lQuests','$lhotkey1','$lhotkey2','$lbuffA','$lBuffTimeA','$lbuffB','$lBuffTimeB','$lbuffC','$lBuffTimeC','$lparty','$lplayerInBattle','$lplayerInAttack','$lplayerInCast','$lpet','$lpethungry','$lpetcare','$lpetTraining','$lpetBath','$lpetLevel');";
		$result = $conn->query($sql);
		if ($conn->query($sql) === TRUE) { echo nl2br("\n - Salvo - \n"); } else { echo nl2br($sql); echo nl2br("\n - Falhou - \n") . $conn->error;}		
	}
